...
//note that this solution is inspired
...
public:
    vector<long long> getDistances(vector<int>& arr) {
        ...
        map<int,vector<int>>mp; //stores positions of same elements
    
        for(int i=0;i<size;i++)
            mp[arr[i]].push_back(i);
       //  pre stores the sum of absolute diff. from the starting index of current element to the current index.
